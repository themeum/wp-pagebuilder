...
class Slider extends Component {
    ...
    render() {
        ...
        let min = 1, max = 6, step = 1;
        if (params.range) {
            min = params.range[dataUnit] ? (params.range[dataUnit].min ? params.range[dataUnit].min : 1) : (params.range.min ? params.range.min : 1)
            max = params.range[dataUnit] ? (params.range[dataUnit].max ? params.range[dataUnit].max : 6) : (params.range.max ? params.range.max : 6)
